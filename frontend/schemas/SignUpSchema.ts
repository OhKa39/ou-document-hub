...
export const SignUpSchema: any = z
  .object({
    ...
    dateOfBirth: z.date().refine(date => {return new Date().getFullYear() - date.getFullYear() >= YOUNGEST && new Date().getFullYear() - date.getFullYear() <= OLDEST}, { message: "Năm sinh không hợp lệ" }),
