export const GET_USER_ENDPOINT = `/api/v1/users/me`;
export const GET_USERS_ENDPOINT = '/api/v1/users';
export const SET_AVATAR_ENDPOINT = `/api/v1/users/me/avatar`;
export const SET_USER_NAME_ENDPOINT = '/api/v1/users/me/name';

export const LOGOUT_ENDPOINT = `/api/v1/auth/logout`;
export const RENEW_REGISTER_TOKEN_ENDPOINT = `/api/v1/auth/renew-register-token`;
export const RENEW_SESSION_ENDPOINT = '/api/v1/auth/renew-session';
export const SIGN_UP_ENDPOINT = '/api/v1/auth/sign-up';
export const SIGN_IN_ENDPOINT = '/api/v1/auth/sign-in';
export const VERIFY_REGISTERTOKEN_ENDPOINT = '/api/v1/auth/verify-register-token';

export const FACULTY_ENDPOINT = '/api/v1/faculties';

export const SHIPPING_ADDRESS_ENDPOINT = '/api/v1/shipping-addresses';

export const DOCUMENT_ENDPOINT = '/api/v1/documents';
